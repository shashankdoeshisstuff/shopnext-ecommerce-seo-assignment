      ...
      {isOpen && (
        <div className="absolute top-16 left-0 right-0 bg-white shadow-lg z-40">
          <nav className="px-4 py-4 flex flex-col gap-4">
            <Link href="/" className="text-neutral-500 hover:text-black" onClick={() => setIsOpen(false)}>
              Home
            </Link>
            <Link href="/store" className="text-neutral-500 hover:text-black" onClick={() => setIsOpen(false)}>
              Store
            </Link>
            <div>
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                {categories.map((cat) => (
                  <Link
                    ...
                    className="text-neutral-600 hover:text-black text-sm"
